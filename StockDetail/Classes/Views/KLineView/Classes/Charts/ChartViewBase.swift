...
open class ChartViewBase: UIView, ChartAnimatorDelegate
{
    ...
    ///滑动偏移位置
    open var globeMatrix : CGAffineTransform = .identity
    ...
    /// if callback the entry when view scrolled
    open var panCallBack : Bool = true
    ...
    open var descriptionTextColor: UIColor! = UtilColor.getTextBlackColor()
    ...
    /// 一屏显示的K线
    open var _countOfCandles : Int = 0
    
    /// 初始数据个数
    open var _countsOfDatas : Int = 0
    
    open var _widthOfCandle : Double = 2
    
    /// 捏合比例
    open var pinchScale : CGFloat {
        set {
            olderScale = 0
            if newValue >= 0 {
                _pinchScale = newValue
            }else{
                _pinchScale = 0
            }
        }
    ...
    fileprivate var _pinchScale : CGFloat = 0
    
    fileprivate var olderScale : CGFloat = -1
    
    /// 蜡烛宽度
    open var widthOfCandle : Double {
        set {
            if newValue < 2 {
                _widthOfCandle = 2
            }else if newValue > 10 {
                ...
                _widthOfCandle = newValue
            }
            let viewTrans = _viewPortHandler.touchOriginMatrix
            _countOfCandles = Int(Double(self.bounds.width) / (_widthOfCandle + 0.1))
            var scaleX = _countOfCandles == 0 ? 1.0 : CGFloat(_countsOfDatas) / CGFloat(_countOfCandles)
            if pinchScale != 0 && (olderScale != pinchScale || olderScale == 0) {
                scaleX = pinchScale
            }else{
                pinchScale = scaleX
                olderScale = scaleX
            ...
            var location = CGPoint(x: self.bounds.width, y: 0)
    ...
    open var data: ChartData?
    {
        ...
        set
        {
            ...
            {
                print("Charts: data argument is nil on setData()")
                return
            }
            
            _countsOfDatas = newValue!.yValCount
            _dataNotSet = false
    ...
    open override func draw(_ rect: CGRect)
    {
        ...
        { // check if there is data
            ...
            ChartUtils.drawText(context: context, text: noDataText, point: CGPoint(x: frame.width / 2.0, y: frame.height / 2.0), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): infoFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): infoTextColor])
            
            if (noDataTextDescription != nil && (noDataTextDescription!).characters.count > 0)
            {   
                let textOffset = -infoFont.lineHeight / 2.0
                
                ChartUtils.drawText(context: context, text: noDataTextDescription!, point: CGPoint(x: frame.width / 2.0, y: frame.height / 2.0 + textOffset), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): infoFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): infoTextColor])
            }
    ...
    internal func drawDescription(context: CGContext)
    {
        ...
        if (font == nil)
        {
            font = UIFont.normalFontOfSize(UIFont.systemFontSize)
        }
    ...
    open var contentRectVolume: CGRect
        {
        return _viewPortHandler.contentRectVolume
    }
    ...
    open var valueFormatter: NumberFormatter!
    {
        get
        {
            _valueFormatter.positiveFormat = "##0.00"
    ...
    open func saveToPath(_ path: String, format: ImageFormat, compressionQuality: Double) -> Bool
    {
        ...
        do{
            _ = try imageData.write(to: URL(string: path)!)
        }catch{
            return false
        }
        return true
