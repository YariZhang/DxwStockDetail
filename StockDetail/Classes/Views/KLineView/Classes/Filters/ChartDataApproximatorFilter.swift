...
open class ChartDataApproximatorFilter: ChartDataBaseFilter
{
    ...
    fileprivate func calcPointToLineDistance(_ startEntry: ChartDataEntry, endEntry: ChartDataEntry, entryPoint: ChartDataEntry) -> Double
    {
        ...
        let normalLength = sqrt((xDiffEndStart)
            ...
            * (endEntry.value - startEntry.value))
        let v1  = xDiffEntryStart * (endEntry.value - startEntry.value) - (entryPoint.value - startEntry.value) * (xDiffEndStart)
        let val = Double(fabs(v1))
        return val / Double(normalLength)
    }
