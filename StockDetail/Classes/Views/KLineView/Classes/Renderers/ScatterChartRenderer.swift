...
open class ScatterChartRenderer: LineScatterCandleRadarChartRenderer
{
    ...
    {
        guard let scatterData = delegate!.scatterChartRendererData(self) else {
            return
        }
        ...
        if ((scatterData?.yValCount)! < Int(ceil(CGFloat(delegate!.scatterChartRendererMaxVisibleValueCount(self)) * viewPortHandler.scaleX)))
        {
            ...
            for i in 0 ..< scatterData!.dataSetCount
            {
                ...
                for j in 0 ..< count
                {
                    ...
                    ChartUtils.drawText(context: context, text: text!, point: CGPoint(x: positions[j].x, y: positions[j].y - shapeSize - lineHeight), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): valueFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): valueTextColor])
                }
            }
        }
    }
