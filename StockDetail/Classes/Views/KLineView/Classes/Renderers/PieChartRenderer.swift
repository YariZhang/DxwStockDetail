...
open class PieChartRenderer: ChartDataRendererBase
{
    ...
    open var centerTextColor = UtilColor.getTextBlackColor()
    open var centerTextFont = UIFont.normalFontOfSize(12.0)
        ...
        for j in 0 ..< entries.count
        {
            ...
            if ((abs(e.value) > 0.000001))
            {
                    ...
                    dataSetIndex: _chart.data!.indexOfDataSet(dataSet)))
                {
                    ...
                    path.addArc(center: CGPoint(x: circleBox.midX, y: circleBox.midY),
                                radius: radius,
                                startAngle: startAngle * ChartUtils.Math.FDEG2RAD,
                                endAngle: endAngle * ChartUtils.Math.FDEG2RAD,
                                clockwise: false)
                    path.closeSubpath()
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        for i in 0 ..< dataSets.count
        {
            ...
            for j in 0 ..< maxEntry
            {
                ...
                {
                    ChartUtils.drawText(context: context, text: val, point: CGPoint(x: x, y: y), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): valueFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): valueTextColor])
                    
                    if (j < data.xValCount && data.xVals[j] != nil)
                    {
                        ChartUtils.drawText(context: context, text: data.xVals[j]!, point: CGPoint(x: x, y: y + lineHeight), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): valueFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): valueTextColor])
                    }
                }
                else if (drawXVals && !drawYVals)
                {
                    ChartUtils.drawText(context: context, text: data.xVals[j]!, point: CGPoint(x: x, y: y + lineHeight / 2.0), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): valueFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): valueTextColor])
                }
                else if (!drawXVals && drawYVals)
                {
                    ChartUtils.drawText(context: context, text: val, point: CGPoint(x: x, y: y + lineHeight / 2.0), align: .center, attributes: [convertFromNSAttributedStringKey(NSAttributedString.Key.font): valueFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): valueTextColor])
                }
