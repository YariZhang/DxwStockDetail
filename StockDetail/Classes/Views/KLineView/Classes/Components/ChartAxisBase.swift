...
{
    open var labelFont = UIFont.normalFontOfSize(10.0)
    open var labelTextColor = HexColor(COLOR_COMMON_BLACK)
    ...
    open var gridColor = HexColor("#ddd")
    ...
    {
        for l in _limitLines
        {
            if (l === line)
            {
                _limitLines.remove(at: _limitLines.index(of: l)!)
                return
            }
        }
    }
