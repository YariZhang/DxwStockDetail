...
open class ChartDataSet: NSObject
{
    ...
    open var valueTextColor: UIColor = UtilColor.getTextBlackColor()
    
    /// the font for the value-text labels
    open var valueFont: UIFont = UIFont.normalFontOfSize(10.0)
    ...
    open func addEntry(_ e: ChartDataEntry , isEnd : Bool = true)
    {
        let val = e.value
    ...
    open func removeEntry(_ entry: ChartDataEntry) -> Bool
    {
        ...
        for e in _yVals
        {
            if (e === entry)
            {
                _yVals.remove(at: _yVals.index(of: e)!)
                removed = true
                break
            }
        }
    ...
    open func colorAt( _ index: Int) -> UIColor
    {
        ...
        return colors[tmpIndex % colors.count]
    }
