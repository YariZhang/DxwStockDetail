//
//  StockIndustryViewController.swift
//  Subject
//
//  Created by zhangyr on 2016/11/11.
...
open class StockIndustryViewController: BaseViewController ,SelectedSegmentedBarDelegate , UITableViewDataSource , UITableViewDelegate , SortHeaderViewDelegate , KLineViewChartDelegate ,TimeLineViewDelegate{
    ...
    //----------------------------------
    fileprivate var stocksInfo : NSArray!
    ...
    fileprivate var segIndex : Int = 0
    fileprivate var sort : Int = 1 // 1 降序 2 升序
    //-----------------------------------
    fileprivate var newsInfos : NSArray!
    ...
    override open func viewDidLoad() {
        ...
        self.tableView.register(ProtfolioTableViewCell.self, forCellReuseIdentifier: "stockCell")
        //self.sb_five = NSArray()
        initNaviBar()
        layoutHeaderView()
        NotificationCenter.default.addObserver(self, selector: #selector(StockIndustryViewController.hideSegmented(_:)), name: NSNotification.Name(rawValue: "moveLine"), object: nil)
    ...
    func initZixuanItem() {
        ...
        var titleStr = "+ 自选"
        var color = Color(COLOR_COMMON_WHITE)
        ...
        refBtn.addTarget(self, action: #selector(StockIndustryViewController.addZixuan(_:)), for: UIControl.Event.touchUpInside)
    ...
    func layoutHeaderView() {
        ...
        rightTop.backgroundColor = Color("#fff")
        ...
        downView.backgroundColor = Color("#fff")
        ...
        let tapGR = UITapGestureRecognizer(target: self, action: #selector(StockIndustryViewController.usingFullScreen(_:)))
        ...
        headerView.frame.size.height = heightCount + 10
    ...
    @objc func usingFullScreen(_ tap : UITapGestureRecognizer) {
        ...
        UIView.animate(withDuration: 0.5, animations: { () -> Void in
            ...
            let panGr       = UIPanGestureRecognizer(target: nil, action: #selector(StockIndustryViewController.closeFullView))
            panGr.delegate  = self
    ...
    func parseJsonData(_ data : NSDictionary) {
        ...
        //dic?.updateValue(name as AnyObject?, forKey: "name")
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if segIndex == 1 {
            ...
            cell.newsInfo = self.newsInfos[(indexPath as NSIndexPath).row] as? NSDictionary
            return cell
        }else{
            let cell = tableView.dequeueReusableCell(withIdentifier: "stockCell", for: indexPath) as! ProtfolioTableViewCell
            cell.stockInfo = stocksInfo[(indexPath as NSIndexPath).row] as? NSDictionary
            cell.checkoutType = 1
            return cell
        }
    }
    ...
    public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let hv = UIView(frame: CGRect(x: 0, y: 0, width: self.tableView.bounds.width, height: height))
        hv.backgroundColor = Color("#edeef0")
        var seg : SelectedSegmentedBar!
        seg = SelectedSegmentedBar(frame: CGRect(x: 0, y: 0, width: hv.bounds.width, height: 38), items: ["个股" as AnyObject,"新闻" as AnyObject], delegate: self)
        seg.tag = 30
        seg.index = segIndex
        hv.addSubview(seg)
        seg.snp.makeConstraints { (maker) in
        ...
        if segIndex == 0 {
            ...
            let orderArr : Array<Dictionary<String , AnyObject>> = [["orderValue" : "股票名称" as AnyObject , "order" : "0" as AnyObject],["orderValue" : "股价" as AnyObject , "order" : "0" as AnyObject],["orderValue" : "涨跌幅" as AnyObject , "order" : "3" as AnyObject , "currentOrder" : sort as AnyObject]]
            sortView.attributes = [[:],["align" : "center" as AnyObject],[:]]
            sortView.showHeader(orderArr)
            sortView.delegate = self
            sortView.backgroundColor    = Color("#f9f9f9")
            hv.addSubview(sortView)
            sortView.snp.makeConstraints({ (maker) in
                maker.left.equalTo(hv)
                maker.right.equalTo(hv)
                maker.top.equalTo(seg.snp.bottom)
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if segIndex == 0 {
            let stock_info = stocksInfo[(indexPath as NSIndexPath).row] as! NSDictionary
            let info = ["name" : stock_info["name"] + "" , "code" : stock_info["code"] + ""]
            self.navigationController?.pushStockDetailController(info as Dictionary<String, AnyObject>, animated: true)
        }else{
            let dic = newsInfos[(indexPath as NSIndexPath).row] as! NSDictionary
            if let para = dic["para"] as? Dictionary<String,AnyObject> {
                let subjectDetail = BaseWebViewController(parameters: para)
                subjectDetail.relativeUrl = dic["url"] + ""
                self.navigationController?.pushViewController(subjectDetail, animated: true)
                return
            }
            let url = "\(ServerType.base.rawValue)stock/news/detail?newsid=%@&res_type=html5"
    ...
    func selectedItemForSegmentedBar(_ segmentedBar: SelectedSegmentedBar, selectedSegmentedIndex: Int) {
        ...
        if segmentedBar.tag == 10 || segmentedBar.tag == 20 {
            ...
            segIndex    = selectedSegmentedIndex
            IndexOfPlateInfo = selectedSegmentedIndex
            let str     = segIndex == 0 ? "gegu" : "xinwen"
    ...
    func parseFiveDayData(_ dic : NSDictionary) {
        ...
        guard let minInfos = dic["min_data"] as? NSDictionary else {
            return
        }
    ...
    func requestList() {
        setupRefresh()
        if segIndex == 0 {
            StockService.getPlateStockList(param?["code"] + "", page: page, sort: sort, completion: { (stockData) in
                self.endRefresh()
                if stockData?.resultCode == 10000 {
                    if let tmpDic = stockData!.resultData as? Dictionary<String , AnyObject> {
                        let tmp = tmpDic["stock_list"] as! NSArray
                        self.sort = tmpDic.getNumberForKey("sort").intValue
                        if self.page == 1 {
                            self.stocksInfo     = tmp
                        }else{
                            let tmpArr          = NSMutableArray(array: self.stocksInfo)
                            tmpArr.addObjects(from: tmp as [AnyObject])
                            self.stocksInfo     = tmpArr
                        }
                        if tmp.count < 10 {
                            self.tableView.mj_footer?.removeFromSuperview()
                            self.tableView.mj_footer = nil
            ...
            StockService.getPlatesNewsWithId(param?["code"] + "" , page: page , completion: { (newsData) in
                ...
                if newsData!.resultCode == 10000 {
                    if let tmpArr = newsData!.resultData as? NSArray {
                        if self.page == 1 {
                            self.newsInfos      = tmpArr
                        }else{
                            let muArr           = NSMutableArray(array: self.newsInfos)
                            muArr.addObjects(from: tmpArr as [AnyObject])
                            self.newsInfos      = muArr
                        }
                        if tmpArr.count < 10 {
                            self.tableView.mj_footer?.removeFromSuperview()
                            self.tableView.mj_footer = nil
